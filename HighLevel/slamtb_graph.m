...
for rob = [Rob.rob]
    ...
    [Rob(rob),Trj(rob),Frm(rob,:),Fac] = addKeyFrame(Rob(rob),...
        Trj(rob),...
        Frm(rob,:),...
        Fac,...
        factorRob(rob),...
...
for currentFrame = Tim.firstFrame : Tim.lastFrame
    ...
    if mod(currentFrame - Tim.firstFrame + 1, Opt.map.kfrmPeriod) == 0
        ...
        for rob = [Rob.rob]
            ...
            % Add key frame
            [Rob(rob),Trj(rob),Frm(rob,:),Fac] = addKeyFrame(Rob(rob),Trj(rob),Frm(rob,:),Fac,factorRob(rob), 'motion');
            
            % Process sensor observations
            for sen = Rob(rob).sensors
                
                % Observe knowm landmarks
                [Rob(rob),Sen(sen),Lmk,Obs(sen,:),Trj(rob),Frm(rob,:),Fac] = addKnownLmkFactors( ...
                    Rob(rob),   ...
                    Sen(sen),   ...
                    Raw(sen),   ...
                    Lmk,        ...
                ...
                for i = 1:ninits
                    [Lmk,Obs(sen,:),Trj(rob),Frm(rob,:),Fac] = addNewLmkFactors(...
                        Rob(rob),   ...
                        Sen(sen),   ...
                        Raw(sen),   ...
                        Lmk,        ...
