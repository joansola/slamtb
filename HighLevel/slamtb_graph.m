...
for rob = [Rob.rob]
    ...
    % Add first frame
    [Trj(rob),Frm(rob,:),Fac] = addFrmToTrj(Trj(rob),Frm(rob,:),Fac);
    
    % Update new frame with Rob info
    Frm(rob,Trj(rob).head) = rob2frm(Rob(rob),Frm(rob,Trj(rob).head));
...
for currentFrame = Tim.firstFrame : Tim.lastFrame
    ...
    if mod(currentFrame - Tim.firstFrame + 1, Opt.map.kfrmPeriod) == 0
        ...
        for rob = [Rob.rob]
            ...
            [Fac(fac), Frm(rob,head_old), Frm(rob,Trj(rob).head)] = makeMotionFactor(Fac(fac), Frm(rob,head_old), Frm(rob,Trj(rob).head), factorRob(rob));
