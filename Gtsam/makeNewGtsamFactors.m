...
for fac = [Facs(neworreplacefac).fac]
    ...
    % NOTE: this value is temporary, and it's used to get the correct value
    % only available after the ISAM2.update() call.
    Facs(fac).gtsamIndex = newFactors.size() - 1;
    
    % NOTE: We do not clear Facs flags yet because it will be needed later
    % to recover the correct 'gtsamIndex' value after the ISAM2 update. See
    % UPDATEGTSAMFACTORINDEXES.
