...
for fac = 1:numFactors
    ...
    Fac(fac).new       = false; % Factor is new? (used by GTSAM)
    Fac(fac).replace   = false; % Factor should be replaced? (used by GTSAM)
    Fac(fac).remove    = false; % Factor should be removed? (used by GTSAM)
    ...
    Fac(fac).err.J4    = []; % Jac. of error wrt. node 4
