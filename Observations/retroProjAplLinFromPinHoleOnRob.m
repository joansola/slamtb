function [l, L_rf, L_sf, L_sk, L_hm, L_beta] = ...
    retroProjAplLinFromPinHoleOnRob(Rf, Sf, Sk, hm, beta)

% RETROPROJAPLLINFROMPINHOLEONROB Retro-project anchored Plucker line from pinhole on robot.
%
%   L = RETROPROJAPLLINFROMPINHOLEONROB(RF, SF, SK, HML, beta) gives the
%   retroprojected anchored Plucker line L in World Frame from an observed
...
if nargout == 1
    ...
    % Plucker line in Sensor Frame
    ls = invPinHoleAPlucker(Sk,hm,beta) ;
    
    % L in world frame
    lr = fromFrameAPlucker(Sf, ls) ;
    l  = fromFrameAPlucker(Rf, lr) ;

else

    % Plk lin in Sensor Frame
    [ls, LS_sk, LS_hm, LS_beta] = invPinHoleAPlucker(Sk,hm,beta) ;
    
    % L in world frame
    [lr, LR_sf, LR_ls] = fromFrameAPlucker(Sf, ls) ;
    [l,  L_rf,  L_lr]  = fromFrameAPlucker(Rf, lr) ;
...
%% test jacobian
[l,L_r,L_s,L_k,L_hm,L_beta] = retroProjAplLinFromPinHoleOnRob(Rf,Sf,Sk,hm,beta)
