...
for rob = [Rob.rob]
    ...
    % Add first frame
    [Trj(rob),Frm(rob,:),Fac] = addFrmToTrj(...
        Trj(rob),...
        Frm(rob,:),...
        Fac);
    
    % Update new frame with Rob info
    [Rob(rob), Frm(rob,Trj(rob).head)] = rob2frm(...
        Rob(rob),...
        Frm(rob,...
        Trj(rob).head));
    ...
    [Frm(rob, Trj(rob).head), Fac(fac)] = makeAbsFactor(...
...
for currentFrame = Tim.firstFrame : Tim.lastFrame
    ...
    if mod(currentFrame - Tim.firstFrame + 1, Opt.map.kfrmPeriod) == 0
        ...
        for rob = [Rob.rob]
            ...
            fac = find([Fac.used] == false, 1, 'first');
            head_old = mod(Trj(rob).head - 2, Trj(rob).maxLength) + 1;
            [Frm(rob,head_old), Frm(rob,Trj(rob).head), Fac(fac)] = makeMotionFactor(Frm(rob,head_old), Frm(rob,Trj(rob).head), Fac(fac), factorRob(rob));
            ...
            for sen = Rob(rob).sensors
                ...
                %                 [Rob(rob),Sen(sen),Lmk,Obs(sen,:)] = observeKnownLmks( ...
                %                     Rob(rob),   ...
                %                     Sen(sen),   ...
                %                     Raw(sen),   ...
                %                     Lmk,        ...
