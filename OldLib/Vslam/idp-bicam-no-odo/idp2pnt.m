...
Pnt.sig     = Idp.sig;
Pnt.Robi    = Idp.Robi;
Pnt.Robi.X(1:3) = idp(1:3);
Pnt.Robi = updateFrame(Pnt.Robi);
Pnt.Cami    = Idp.Cami;   % Initial camera
Pnt.ui      = Idp.ui;   % Initial detected pixel
Pnt.vis0    = Idp.vis0;
Pnt.dUmax   = Idp.dUmax;
...
for cam = 1:2
    ...
    Pnt.Prj(cam).sr      = Idp.Prj(cam).sr;
    Pnt.Prj(cam).d       = Idp.Prj(cam).d;
    Pnt.Prj(cam).dr      = Idp.Prj(cam).dr;
    Pnt.Prj(cam).u       = Idp.Prj(cam).u;
    Pnt.Prj(cam).U       = Idp.Prj(cam).U;
    Pnt.Prj(cam).dU      = Idp.Prj(cam).dU;
    Pnt.Prj(cam).z       = Idp.Prj(cam).z;
    Pnt.Prj(cam).Z       = Idp.Prj(cam).Z;
    Pnt.Prj(cam).iZ      = Idp.Prj(cam).iZ;
    Pnt.Prj(cam).MD      = Idp.Prj(cam).MD;
    Pnt.Prj(cam).region  = Idp.Prj(cam).region;
    Pnt.Prj(cam).cPatch  = Idp.Prj(cam).cPatch;
    Pnt.Prj(cam).wPatch  = Idp.Prj(cam).wPatch;
