...
H(:, 1)    = zSegment   (x  ,y  ,z,z+r*h); % 4 walls
H(:, 2)    = zSegment   (x+l,y  ,z,z+r*h);
H(:, 3)    = zSegment   (x+l,y+w,z,z+r*h);
H(:, 4)    = zSegment   (x  ,y+w,z,z+r*h);
H(:,5:8)   = XYRectangle(x,x+l,y,y+w,z); % floor
H(:, 9)    = makeSegment([x;y    ;z+r*h],[x;y+w/2;z+h    ]); % roof slopes
...
H(:,13)    = xSegment   (x,x+l   ,y+.5*w,z+h);   % roof top
H(:,14)    = xSegment   (x,x+l   ,y     ,z+r*h); % roof bot
H(:,15)    = xSegment   (x,x+l   ,y+w   ,z+r*h); % roof bot
H(:,16:19) = YZRectangle(x,y+c*w,y+d*w,z,z+q*h); % door
H(:,20:23) = YZRectangle(x,y+a*w,y+b*w,z+p*h,z+q*h); % window
